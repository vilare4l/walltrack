...
### Database Migration (add to V8)

**migrations/V8__config_tables.sql (addition):**
```sql
...
CREATE TABLE IF NOT EXISTS exit_strategies (
    ...
    stagnation_hours INTEGER NOT NULL DEFAULT 6,
    stagnation_threshold_pct DECIMAL(5,2) NOT NULL DEFAULT 2.0,
    ...
    CONSTRAINT exit_strategies_unique_active
        EXCLUDE USING btree (name WITH =) WHERE (status = 'active')
);

-- Indexes
CREATE INDEX idx_exit_strategies_name ON exit_strategies(name);
CREATE INDEX idx_exit_strategies_status ON exit_strategies(status);
CREATE INDEX idx_exit_strategies_name_status ON exit_strategies(name, status);

-- Updated_at trigger
CREATE TRIGGER update_exit_strategies_updated_at
    BEFORE UPDATE ON exit_strategies
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Default strategies
INSERT INTO exit_strategies (name, description, version, status, rules, activated_at)
VALUES
(
    'Standard',
    'Standard exit strategy with 15% TP, 10% SL',
    1,
    'active',
    '[
        {"rule_type": "take_profit", "trigger_pct": 15, "exit_pct": 50, "priority": 1, "enabled": true, "params": {}},
        {"rule_type": "take_profit", "trigger_pct": 30, "exit_pct": 100, "priority": 2, "enabled": true, "params": {}},
        {"rule_type": "stop_loss", "trigger_pct": -10, "exit_pct": 100, "priority": 0, "enabled": true, "params": {}},
        {"rule_type": "trailing_stop", "trigger_pct": -5, "exit_pct": 100, "priority": 3, "enabled": true, "params": {"activation_pct": 10}}
    ]',
    NOW()
),
(
    'High Conviction',
    'Aggressive exit for high conviction trades: 25% TP, 15% SL',
    1,
    'active',
    '[
        {"rule_type": "take_profit", "trigger_pct": 25, "exit_pct": 30, "priority": 1, "enabled": true, "params": {}},
        {"rule_type": "take_profit", "trigger_pct": 50, "exit_pct": 50, "priority": 2, "enabled": true, "params": {}},
        {"rule_type": "take_profit", "trigger_pct": 100, "exit_pct": 100, "priority": 3, "enabled": true, "params": {}},
        {"rule_type": "stop_loss", "trigger_pct": -15, "exit_pct": 100, "priority": 0, "enabled": true, "params": {}},
        {"rule_type": "trailing_stop", "trigger_pct": -8, "exit_pct": 100, "priority": 4, "enabled": true, "params": {"activation_pct": 20}}
    ]',
    NOW()
),
(
    'Conservative',
    'Conservative exit: tight stops, quick profits',
    1,
    'active',
    '[
        {"rule_type": "take_profit", "trigger_pct": 10, "exit_pct": 75, "priority": 1, "enabled": true, "params": {}},
        {"rule_type": "take_profit", "trigger_pct": 20, "exit_pct": 100, "priority": 2, "enabled": true, "params": {}},
        {"rule_type": "stop_loss", "trigger_pct": -5, "exit_pct": 100, "priority": 0, "enabled": true, "params": {}},
        {"rule_type": "time_based", "trigger_pct": null, "exit_pct": 100, "priority": 5, "enabled": true, "params": {"max_hours": 12}}
    ]',
    NOW()
);
```
...
- [ ] Create ExitStrategyRule model
- [ ] Create ExitStrategyCreate/Update models
- [ ] Create ExitStrategy full model
- [ ] Implement ExitStrategyService class
- [ ] Implement create() with versioning
- [ ] Implement update() with draft/active handling
- [ ] Implement activate() with archive of previous
- [ ] Implement clone()
- [ ] Add database migration for exit_strategies table
...
- [ ] Versioning creates new versions on active edit
...
### Modified Files
- `migrations/V8__config_tables.sql` - Add exit_strategies table
