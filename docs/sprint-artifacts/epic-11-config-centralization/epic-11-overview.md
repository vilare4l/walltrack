...
### 11-1: Schema et Migration des Tables de Config
...
- **When** je liste les tables
- **Then** je vois 8 tables de config:
  - `trading_config`
  - `scoring_config`
  - `discovery_config`
  - `cluster_config`
  ...
  - `api_config`
  - `config_audit_log`
- **And** chaque table a les colonnes:
  - `id` (PK)
  - `status` (enum: default, draft, active, archived)
  - `name` (identifiant unique pour cette config)
  - `version` (numéro auto-incrémenté)
  - `created_at`, `updated_at`, `created_by`, `updated_by`
- **And** les valeurs par défaut sont insérées avec `status = 'default'`
- **And** une ligne `status = 'active'` existe (clone des defaults)

**Technical Notes:**
- Fichier: `migrations/V8__config_centralization.sql`
- Contrainte: une seule config `active` par table
...
### 11-7: Exit Strategy - CRUD et Versioning
...
- **Given** la table `exit_strategies`
- **When** je la consulte
- **Then** je vois les 5 presets avec `status = 'default'`
- **And** je peux voir mes stratégies custom avec leur status
...
- **And** je peux la définir comme "default pour high conviction"

- **Given** je modifie une stratégie active
- **When** je sauvegarde
- **Then** l'ancienne version est archivée
- **And** la nouvelle version devient active

**Technical Notes:**
- Étendre la table `exit_strategies` avec:
  - `status` (default, draft, active, archived)
  - `version`
  - `parent_id` (pour tracer les clones)

**Story Points:** 5

---

### 11-8: Exit Strategy - Simulation Engine
...
- **Then** je vois un tableau:
  | Strategy | Simulated P&L | Actual P&L | Delta |
  |----------|---------------|------------|-------|
  | Conservative | +45% | +30% | +15% |
  | Balanced | +38% | +30% | +8% |
  | Quick Flip | +22% | +30% | -8% |

- **Given** une simulation terminée
- **When** une stratégie performe mieux
- **Then** je peux l'activer en un clic
...
- Fichier: `src/walltrack/services/exit_simulator.py`
- Données requises: `price_history` pour la position (via DexScreener API)
