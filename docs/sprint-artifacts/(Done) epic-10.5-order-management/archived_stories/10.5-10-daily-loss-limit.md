...
- [x] Create DailyLossTracker class
- [x] Implement realized/unrealized P&L calculation
- [x] Create daily_snapshots migration
- [ ] Create daily snapshot job (deferred to scheduled jobs epic)
- [x] Integrate with PositionSizer.calculate_size (Step 0)
- [x] Add warning zone detection at 80% threshold
- [ ] Create Gradio daily_pnl panel (deferred to Story 10.5-13)
- [ ] Add auto-refresh for daily P&L display (deferred to Story 10.5-13)
- [x] Write unit tests (16 tests)

## Definition of Done

- [x] Daily P&L calculated correctly (realized + unrealized)
- [x] Entries blocked when limit hit
- [x] Exits still allowed when blocked (via decision type)
- [x] Midnight UTC resets the counter (via date-based calculation)
- [x] Warning at 80% of limit (warning_zone detection)
- [ ] Alert created when limit hit (deferred to Story 10.5-14)
- [ ] Gradio shows clear status (deferred to Story 10.5-13)
