...
- [x] Create BacktestParameters model
- [x] Create BacktestResult and BacktestTrade models
- [x] Implement BacktestEngine.run()
- [x] Implement signal rescoring logic
- [x] Implement trade simulation with historical prices
- [x] Implement stop-loss and take-profit checks
- [x] Calculate backtest metrics
- [x] Generate equity curve
- [x] Write unit tests with mock data

## Definition of Done

- [x] Backtest engine replays signals correctly
- [x] Scoring uses provided parameters
- [x] Trade simulation uses historical prices
- [x] Exit strategies are applied correctly
- [x] Metrics are calculated accurately
- [x] Equity curve is generated
- [x] Tests cover all scenarios

## Dev Agent Record

### Implementation Notes
- Created `parameters.py` with ScoringWeights, ExitStrategyParams, BacktestParameters models
- ScoringWeights includes validate_weights() to ensure sum to 1.0
- ExitStrategyParams configures stop-loss, take-profit levels, trailing stop, moonbag
- Created `results.py` with BacktestTrade, BacktestMetrics, BacktestResult models
- BacktestTrade has computed property `is_winner` for P&L assessment
- BacktestMetrics has `calculate_from_trades()` for aggregate statistics
- Created `engine.py` with BacktestEngine class
- Engine processes signals chronologically, rescores with configurable weights
- Implements position limits, slippage, exit strategies
- Generates equity curve and calculates max drawdown

### Tests Created
- `tests/unit/core/backtest/test_parameters.py` - 7 tests covering:
  - ScoringWeights default values and validation
  - ExitStrategyParams defaults and custom values
  - BacktestParameters creation with defaults and custom values
- `tests/unit/core/backtest/test_results.py` - 6 tests covering:
  - BacktestTrade creation, winning/losing trade states
  - BacktestMetrics default values and calculate_from_trades
  - BacktestResult creation
- `tests/unit/core/backtest/test_engine.py` - 6 tests covering:
  - BacktestEngine.run() returns result
  - Signal processing and trading
  - Rescoring with custom weights
...
### New Files
- `src/walltrack/core/backtest/parameters.py` - Backtest parameters models
- `src/walltrack/core/backtest/results.py` - Result and trade models
- `src/walltrack/core/backtest/engine.py` - Core backtest engine
- `tests/unit/core/backtest/test_parameters.py` - Parameters tests
- `tests/unit/core/backtest/test_results.py` - Results tests
- `tests/unit/core/backtest/test_engine.py` - Engine tests

## Change Log

- 2025-12-21: Story 8-2 implementation complete - Backtest engine core
