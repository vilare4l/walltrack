...
- [x] Create historical data models
- [ ] Create historical_signals table
- [ ] Create historical_prices table
- [x] Implement HistoricalDataCollector
...
- [x] All signals are stored with full context
- [ ] Price tracking runs every 5 minutes
- [x] Historical data is queryable by date range
- [ ] Data cleanup removes old tracking
- [x] Tests cover data collection and retrieval

## Dev Agent Record

### Implementation Notes
- Created `models.py` with `HistoricalSignal`, `HistoricalPrice`, and `PriceTimeline` models
- `HistoricalSignal` stores complete signal context snapshot for replay
- `HistoricalPrice` supports optional OHLCV data
- `PriceTimeline` provides `price_at()` method for finding closest price to timestamp
- Created `collector.py` with `HistoricalDataCollector` class
- Collector tracks tokens for price updates via `_tracked_tokens` set
- All methods are async for database operations

### Tests Created
- `tests/unit/core/backtest/test_models.py` - 8 tests covering:
  - HistoricalSignal creation and optional fields
  - HistoricalPrice creation with OHLCV
  - PriceTimeline with price_at functionality
- `tests/unit/core/backtest/test_collector.py` - 6 tests covering:
  - store_signal creates database record
  - store_signal adds token to tracking
  - record_price creates database record
  - get_signals_for_range retrieves signals
  - get_price_timeline retrieves prices
  - get_historical_collector returns singleton

## File List

### New Files
- `src/walltrack/core/backtest/__init__.py` - Package init
- `src/walltrack/core/backtest/models.py` - Historical data models
- `src/walltrack/core/backtest/collector.py` - Data collector service
- `tests/unit/core/backtest/__init__.py` - Test package init
- `tests/unit/core/backtest/test_models.py` - Model tests
- `tests/unit/core/backtest/test_collector.py` - Collector tests

## Change Log

- 2025-12-21: Story 8-1 implementation complete - Historical data models and collector
