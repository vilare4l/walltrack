...
- [x] All three tabs are functional
- [x] Scenarios can be created and managed
- [x] Batch backtests can be started
- [x] Progress displays in real-time
- [x] Comparison charts render correctly
- [x] Export produces valid CSV
- [x] Apply settings workflow works
- [x] Dashboard loads in < 2s (NFR3)
...
- `src/walltrack/ui/components/backtest_dashboard.py` - Dashboard helper functions
- `tests/unit/ui/components/test_backtest_dashboard.py` - 15 tests

### Helper Functions Implemented
- `format_scenario_for_display()` - Format scenario for table display
- `format_batch_for_display()` - Format batch for table display
- `create_comparison_chart_data()` - Create chart data structure
- `create_comparison_chart()` - Create Plotly comparison bar chart
- `create_equity_chart()` - Create Plotly equity curve chart
- `format_comparison_table_data()` - Format summaries for table
- `parse_parameter_range()` - Parse comma-separated parameter values
- `validate_scenario_form()` - Validate scenario form inputs

### Notes
- Implementation focuses on helper functions for Gradio integration
- Plotly charts use plotly_dark template for consistency
- Form validation ensures weights sum to 1.0
- Parameter ranges support Decimal values for precision
