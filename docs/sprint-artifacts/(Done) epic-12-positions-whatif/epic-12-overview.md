...
**⚠️ Status: DONE** - Implémenté via Story 10.5-7 (Price History Infrastructure)

L'infrastructure de price history existe déjà:
- `migrations/V10__price_history.sql` - Tables partitionnées
- `migrations/V11__daily_snapshots.sql` - Snapshots journaliers
- `src/walltrack/services/pricing/price_oracle.py` - Oracle de prix
- `src/walltrack/data/supabase/repositories/price_history_repo.py` - Repository

Pas d'implémentation additionnelle requise.

**Story Points:** 0 (déjà implémenté)

---

### 12-5: Exit Simulator - Position-Level Wrapper

**⚠️ Important:** Cette story NE réimplémente PAS le simulateur - elle crée un wrapper autour de `ExitSimulationEngine` (Story 11-8).
...
- **Given** un position_id et une stratégie
...
- **Given** une liste de positions
- **When** je lance une simulation batch
- **Then** chaque position est simulée et les résultats agrégés
...
class PositionSimulator:
    ...
    async def simulate_by_id(
        ...
        """Load position and delegate to ExitSimulationEngine."""
...
- Fichier: `src/walltrack/services/simulation/position_simulator.py`
- Dépendance: `src/walltrack/services/exit/simulation_engine.py` (Story 11-8)

**Story Points:** 3 (wrapper uniquement)
...
| 12-4: Price History Collection | 0 | ✅ DONE | Via 10.5-7 |
| 12-5: Exit Simulator Wrapper | 3 | P0 | Wrapper pour 11-8 |
...
| **Total** | **50** | | (was 63, -13 avec corrections) |
...
## Dépendances avec Epic 10.5 et 11

| Epic 12 Story | Dépend de |
|---------------|-----------|
| 12-3: Change Strategy | 11-7: Exit CRUD (liste des stratégies) |
| 12-4: Price History | ✅ 10.5-7: Price History Infrastructure (DONE) |
| 12-5: Simulator Wrapper | **11-8: ExitSimulationEngine** (core logic) |
| 12-6: Comparison Logic | 12-5: PositionSimulator |
| 12-7: What-If Modal | 11-9: Exit Strategy Editor (pour voir/créer) |
| 12-9: Global Analysis | 12-5: PositionSimulator.batch_simulate() |

**Ordre d'implémentation recommandé:**
1. Epic 11 Phase 3: 11-7, **11-8** (Exit CRUD + Simulation Engine)
2. Epic 12: 12-5 (wrapper), puis UI stories

**Recommandation:** Faire Epic 11 Phase 1-3 avant Epic 12.
