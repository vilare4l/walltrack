...
- [x] Create SimulationLogger class
- [ ] Create simulation_events table
- [x] Implement trade logging
- [x] Implement circuit breaker logging
- [x] Create daily summary generator
- [x] Implement Discord/Telegram alerts
- [ ] Add scheduled daily summary job
- [x] Write unit tests

## Definition of Done

- [x] All simulation events are logged
- [x] Circuit breaker tracking works
- [x] Daily summary generates correctly
- [x] Alerts are sent via configured channels
...
- Created `logger.py` with `SimulationEventType` enum and `SimulationLogger` class
- Implemented event types: TRADE_EXECUTED, POSITION_OPENED, POSITION_CLOSED, STOP_LOSS_HIT, TAKE_PROFIT_HIT, CIRCUIT_BREAKER, SIGNAL_PROCESSED, DAILY_SUMMARY
- Logger stores events in `simulation_events` table with `simulated=True` flag
- Created `daily_summary.py` with:
  - `generate_daily_summary()` - calculates wins, losses, win_rate, P&L from daily trades
  - `send_summary_alert()` - sends formatted summary via Discord/Telegram
  - `_format_summary_message()` - readable message formatting
- Both Discord and Telegram alert functions use httpx for async HTTP calls

### Tests Created
- `tests/unit/core/simulation/test_logger.py` - 6 tests covering:
  - SimulationEventType enum existence and string values
  - log_trade stores event correctly
  - log_circuit_breaker stores event correctly
  - log_signal_processed stores event correctly
  - get_simulation_logger returns singleton
- `tests/unit/core/simulation/test_daily_summary.py` - 3 tests covering:
  - generate_daily_summary returns expected keys
  - Win rate calculation (75% for 3 wins, 1 loss)
  - send_summary_alert formats message without error

## File List

### New Files
- `src/walltrack/core/simulation/logger.py` - Structured simulation event logging
- `src/walltrack/core/simulation/daily_summary.py` - Daily summary generation and alerts
- `tests/unit/core/simulation/test_logger.py` - Logger unit tests
- `tests/unit/core/simulation/test_daily_summary.py` - Daily summary unit tests

## Change Log

- 2025-12-21: Story 7-6 implementation complete - Simulation alerts and logging
