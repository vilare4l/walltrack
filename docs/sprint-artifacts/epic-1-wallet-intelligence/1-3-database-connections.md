...
> See `docker-compose.override.yml` for network configuration.

**Dedicated Schemas/Databases:**

WallTrack uses isolated namespaces to separate data from other projects sharing the same infrastructure:

| Database | Isolation | Environment Variable | Value |
|----------|-----------|---------------------|-------|
| **Neo4j** | Dedicated database | `NEO4J_DATABASE` | `walltrack` |
| **PostgreSQL** | Dedicated schema | `POSTGRES_SCHEMA` | `walltrack` |

**Connection Modes:**

| Mode | Neo4j URI | PostgreSQL Host | Use Case |
|------|-----------|-----------------|----------|
| **Local Dev** | `bolt://localhost:7687` | `localhost:54322` | Running outside Docker |
| **Docker (localai)** | `bolt://localai-neo4j-1:7687` | `localai-postgres-1:5432` | Running in Docker with localai stack |

**Container Names (localai stack):**
- `localai-neo4j-1` - Graph database (port 7687)
- `localai-postgres-1` - PostgreSQL via Supabase (port 5432)
- `localai-redis-1` - Redis cache (port 6379, optional)
...
NEO4J_DATABASE=walltrack           # Dedicated database

# PostgreSQL configuration
POSTGRES_SCHEMA=walltrack          # Dedicated schema

# Docker override (in docker-compose.override.yml)
# NEO4J_URI=bolt://localai-neo4j-1:7687
```

**Schema Initialization:**

The clients automatically create the database/schema if they don't exist:
- **Neo4j**: Creates database `walltrack` on first connection (requires Neo4j 4.0+)
- **PostgreSQL**: Creates schema `walltrack` and sets `search_path` for all queries
...
- [ ] Create `src/walltrack/data/neo4j/client.py`
- [ ] Create `src/walltrack/data/neo4j/__init__.py`
- [ ] Create `src/walltrack/data/supabase/client.py`
- [ ] Create `src/walltrack/data/supabase/__init__.py`
- [ ] Create `src/walltrack/services/base.py` with BaseAPIClient
- [ ] Create `src/walltrack/api/app.py` with lifespan
- [ ] Create `src/walltrack/api/routes/health.py`
- [ ] Write integration tests for Neo4j connection
- [ ] Write integration tests for Supabase connection
- [ ] Verify health check endpoint works

## Definition of Done

- [ ] Neo4j connects and executes queries
- [ ] Supabase connects and executes queries
