...
- [x] Create `src/walltrack/services/dexscreener/client.py`
- [x] Implement DexScreener API client
- [x] Add response caching (5 min TTL)
- [x] Create Birdeye fallback client
- [x] Implement fallback logic
- [x] Handle new tokens gracefully
- [x] Attach data to signal context

## Definition of Done

- [x] Token data fetched from DexScreener
- [x] Caching works with 5 min TTL
- [x] Fallback to Birdeye on failure
- [x] New tokens handled gracefully
...
1. **`src/walltrack/constants/token.py`** - Token fetching constants (cache TTL, API timeouts, thresholds)
2. **`src/walltrack/models/token.py`** - Token domain models (TokenSource, TokenCharacteristics, TokenFetchResult, etc.)
3. **`src/walltrack/services/dexscreener/client.py`** - DexScreener API client with tenacity retry logic
4. **`src/walltrack/services/dexscreener/__init__.py`** - Updated exports
5. **`src/walltrack/services/birdeye/client.py`** - Birdeye fallback client (NFR18 compliance)
6. **`src/walltrack/services/birdeye/__init__.py`** - Updated exports
7. **`src/walltrack/services/token/cache.py`** - In-memory LRU cache with TTL (O(1) lookups)
8. **`src/walltrack/services/token/fetcher.py`** - Token fetcher with fallback chain (Cache → DexScreener → Birdeye → Neutral)
9. **`src/walltrack/services/token/__init__.py`** - Module exports

### Test Files:

1. **`tests/unit/services/token/__init__.py`** - Test module init
2. **`tests/unit/services/token/test_models.py`** - 17 tests for domain models
3. **`tests/unit/services/token/test_cache.py`** - 12 tests for cache operations (TTL, LRU, stats)
4. **`tests/unit/services/token/test_fetcher.py`** - 12 tests for fetcher with fallback scenarios
...
- **AC1**: DexScreener query with retry logic (3 attempts, exponential backoff)
- **AC2**: 5-minute TTL cache with LRU eviction (max 5000 entries)
- **AC3**: Fallback chain: DexScreener → Birdeye → Stale Cache → Neutral defaults
- **AC4**: New token flag for tokens < 10 minutes old
