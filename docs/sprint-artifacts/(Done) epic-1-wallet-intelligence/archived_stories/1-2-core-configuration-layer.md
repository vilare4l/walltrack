...
- [x] Create `src/walltrack/config/settings.py` with full Settings model
- [x] Create `src/walltrack/config/logging.py` with structlog configuration
- [x] Create `src/walltrack/config/__init__.py` with exports
- [x] Create `src/walltrack/api/dependencies.py` with settings dependency
- [x] Update `.env.example` with all variables
- [x] Create `src/walltrack/data/models/config.py` for DynamicConfig model
- [x] Create `src/walltrack/data/supabase/repositories/config_repo.py`
- [x] Add config table to Supabase migration
- [x] Write unit tests for settings validation
- [x] Verify secrets are never logged

## Definition of Done

- [x] Settings load correctly from .env
- [x] Missing required fields raise ValidationError
- [x] SecretStr values never appear in logs or repr
- [x] mypy passes for all settings usage
- [x] Dynamic config can be loaded from Supabase
