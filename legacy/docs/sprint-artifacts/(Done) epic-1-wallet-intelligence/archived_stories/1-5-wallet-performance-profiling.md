...
- [x] Create `src/walltrack/discovery/profiler.py`
- [x] Implement win rate calculation
- [x] Implement PnL calculation
- [x] Implement timing percentile calculation
- [x] Extract behavioral patterns (hours, size, preferences)
- [x] Store profile in Supabase
- [x] Handle insufficient data gracefully
- [x] Add profile API endpoints
- [x] Create scheduled profiling task
- [x] Write unit tests

## Definition of Done

- [x] Profiler calculates all metrics accurately
- [x] Insufficient data handled gracefully
- [x] Profile stored in database
- [x] Metrics retrievable for dashboard display
...
### Files Created
- `src/walltrack/discovery/profiler.py` - WalletProfiler with async profiling engine
- `src/walltrack/scheduler/tasks/profiling_task.py` - Scheduled profiling task
- `src/walltrack/api/routes/wallets.py` - Profile endpoints (`POST /wallets/{address}/profile`)

### Tests
- `tests/unit/discovery/test_profiler.py` - 15 tests for profiler
- Tests cover: win rate calculation, PnL calculation, insufficient data handling, batch profiling

### Key Implementation Details
- WalletProfiler class with configurable minimum trade threshold (default: 5)
- Calculates: win_rate, total_pnl, avg_pnl_per_trade, timing_percentile, avg_hold_time_hours, preferred_hours, avg_position_size_sol
- Sets status to INSUFFICIENT_DATA when trades < min_trades
- Batch profiling with configurable concurrency
- Profile stored via WalletRepository.update_profile()
