...
- [x] Create signal filter module
- [x] Implement efficient wallet lookup (< 50ms)
- [x] Add in-memory cache for monitored wallets
- [x] Integrate blacklist check
- [x] Log discarded signals at DEBUG level
- [x] Pass filtered signals to scoring pipeline

## Definition of Done

- [x] Only monitored wallet signals proceed
- [x] Lookup time < 50ms
- [x] Blacklisted wallets blocked
- [x] Non-monitored signals discarded efficiently
...
**New Files:**
- `src/walltrack/constants/signal_filter.py` - Filter constants (MAX_LOOKUP_TIME_MS, CACHE_TTL, etc.)
- `src/walltrack/models/signal_filter.py` - Domain models (FilterStatus, WalletCacheEntry, FilterResult, SignalContext)
- `src/walltrack/services/signal/__init__.py` - Module exports
- `src/walltrack/services/signal/wallet_cache.py` - LRU cache with O(1) lookups
- `src/walltrack/services/signal/filter.py` - Signal filter service
- `src/walltrack/services/signal/pipeline.py` - Signal processing pipeline

**Test Files:**
- `tests/unit/services/signal/__init__.py`
- `tests/unit/services/signal/test_models.py` - 19 tests for domain models
- `tests/unit/services/signal/test_wallet_cache.py` - 15 tests for cache behavior
- `tests/unit/services/signal/test_filter.py` - 12 tests for filter logic
- `tests/unit/services/signal/test_pipeline.py` - 7 tests for pipeline

**Modified:**
- `src/walltrack/api/routes/webhooks.py` - Integrated signal pipeline in background task

### Test Results

- **53 tests passed** (0 failed)
- All acceptance criteria validated:
  - AC1: Wallet lookup < 50ms (in-memory O(1) via sets)
  - AC2: Monitored wallets pass with metadata enrichment
  - AC3: Non-monitored wallets discarded at DEBUG level
  - AC4: Blacklisted wallets blocked at INFO level

### Architecture Notes

- `WalletCache` uses `_monitored_set` and `_blacklist_set` for O(1) membership checks
- LRU eviction via `OrderedDict` when cache exceeds `max_size`
- TTL-based expiration with automatic refresh
- Pipeline singleton pattern with lazy initialization for dependencies
