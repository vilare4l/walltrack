...
- [x] Create `src/walltrack/api/routes/webhooks.py`
- [x] Implement HMAC signature validation middleware
- [x] Parse Helius webhook payload
- [x] Extract transaction details
- [x] Pass to signal processing pipeline (stub - Story 3-2+)
- [x] Add health check endpoint
- [x] Ensure < 500ms processing time

## Definition of Done

- [x] Webhook endpoint receives Helius notifications
- [x] HMAC validation rejects invalid requests
- [x] Payload parsed correctly
- [x] Processing time < 500ms (verified by unit tests)
...
- `src/walltrack/constants/webhook.py` - Webhook constants (DEX programs, HMAC settings)
- `src/walltrack/constants/__init__.py` - Updated exports
- `src/walltrack/services/helius/models.py` - Webhook data models (ParsedSwapEvent, etc.)
- `src/walltrack/services/helius/webhook_manager.py` - WebhookParser service
- `src/walltrack/services/helius/__init__.py` - Updated exports
- `src/walltrack/api/middleware/hmac_validation.py` - HMAC validation middleware
- `src/walltrack/api/routes/webhooks.py` - Webhook API routes
- `src/walltrack/data/supabase/repositories/webhook_repo.py` - Webhook repository
- `src/walltrack/data/supabase/repositories/__init__.py` - Updated exports
- `src/walltrack/data/supabase/migrations/006_webhook_logs.sql` - Database migration
- `src/walltrack/api/app.py` - Added HMAC middleware
- `pyproject.toml` - Added B008 to ruff ignores (FastAPI pattern)

### Tests Created
- `tests/unit/services/helius/test_models.py` - Model validation tests (14 tests)
- `tests/unit/services/helius/test_webhook_parser.py` - Parser tests (11 tests)
- `tests/unit/api/test_hmac_middleware.py` - HMAC validation tests (10 tests)

### Test Results
- **35/35 tests passed** in 1.22s
- Performance test confirms parsing < 50ms average (well under 500ms NFR2)

### Notes
- Signal pipeline integration is stubbed - will be connected in Story 3-2+
- B008 lint rule ignored for FastAPI Depends pattern (standard practice)
- Used `datetime.UTC` alias per ruff UP017 recommendation
