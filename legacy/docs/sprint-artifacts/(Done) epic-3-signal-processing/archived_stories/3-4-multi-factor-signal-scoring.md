...
- [x] Create `src/walltrack/services/scoring/signal_scorer.py`
- [x] Implement wallet score calculation
- [x] Implement cluster score calculation
- [x] Implement token score calculation
- [x] Implement context score calculation
- [x] Calculate weighted final score
- [x] Preserve factor contributions
- [x] Make weights configurable

## Definition of Done

- [x] All four factors calculated correctly
- [x] Weighted score between 0.0 and 1.0
- [x] Factor contributions preserved
- [x] Weights configurable via config
...
1. **`src/walltrack/constants/scoring.py`** - Scoring constants (weights, thresholds, peak hours)
2. **`src/walltrack/models/scoring.py`** - Scoring domain models (FactorScore, ScoredSignal, ScoringWeights, etc.)
3. **`src/walltrack/services/scoring/__init__.py`** - Module exports
4. **`src/walltrack/services/scoring/signal_scorer.py`** - Multi-factor signal scoring engine

### Test Files:

1. **`tests/unit/services/scoring/__init__.py`** - Test module init
2. **`tests/unit/services/scoring/test_models.py`** - 16 tests for scoring models
3. **`tests/unit/services/scoring/test_signal_scorer.py`** - 24 tests for scoring engine

### Test Results:

- **40 tests passing**
- All acceptance criteria covered
- Comprehensive coverage for all four scoring factors

### Key Features:

- **AC1**: Four factor scores calculated (wallet 30%, cluster 25%, token 25%, context 20%)
- **AC2**: Wallet score with win rate, PnL, timing, consistency, leader bonus, decay penalty
- **AC3**: Cluster score with amplification factor (protocol-based for Epic 2 integration)
- **AC4**: Token score with liquidity, market cap, holder distribution, new token penalty, honeypot risk
- **AC5**: Context score with time-of-day patterns and market volatility (placeholder)
- **AC6**: Weighted average produces score 0.0-1.0, individual contributions preserved

### Architecture:

- Protocol-based cluster amplifier integration for Epic 2
