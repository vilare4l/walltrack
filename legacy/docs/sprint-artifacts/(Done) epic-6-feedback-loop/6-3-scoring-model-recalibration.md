...
- [x] Create `src/walltrack/core/feedback/model_calibrator.py`
- [x] Implement factor-to-outcome correlation analysis
- [x] Generate weight suggestions
- [x] Compare to current weights
- [x] Apply approved weights
- [x] Archive previous weights
- [x] (Optional) Implement auto-recalibration mode

## Definition of Done

- [x] Correlation analysis calculates factor effectiveness
- [x] Weight suggestions generated
- [x] Operator can approve or modify suggestions
- [x] Previous weights archived
...
### Files Created
- `src/walltrack/core/feedback/calibration_models.py` - Pydantic models for calibration
- `src/walltrack/core/feedback/model_calibrator.py` - ModelCalibrator service (793 lines)
- `src/walltrack/api/routes/calibration.py` - FastAPI routes for calibration endpoints
- `src/walltrack/data/supabase/migrations/013_calibration.sql` - Database schema
- `tests/core/feedback/test_model_calibrator.py` - 22 unit tests

### Test Results
All 22 tests passing:
- TestCalibrationModels: 5 tests (WeightSet normalization, FactorCorrelation predictiveness)
- TestFactorCorrelation: 3 tests (correlation calculation, winner/loser averages)
- TestWeightGeneration: 2 tests (normalized weights, correlation-based weighting)
- TestWeightBounding: 3 tests (bounds large increases/decreases, small changes pass)
- TestImprovementEstimation: 2 tests (improvement estimation, score calculation)
- TestWeightApplication: 4 tests (apply approved, apply modified, reject, not found)
- TestRationale: 3 tests (predictive factor, weak factor, without correlation)

### Key Features
- Pearson correlation analysis for each scoring factor
- Conservative weight blending (70% correlation-based, 30% current)
- Maximum weight change bounds per calibration
- Auto-calibration mode with configurable thresholds
- Weight archiving for historical tracking
- API endpoints for analysis, suggestion, and application
