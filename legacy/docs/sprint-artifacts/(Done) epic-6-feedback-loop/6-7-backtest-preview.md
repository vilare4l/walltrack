...
- [x] Backtest configurable with multiple parameters
- [x] Historical signals re-scored correctly
- [x] Comparison shows actual vs simulated
- [x] Settings can be applied after review
- [x] Progress indicator during execution
- [x] Completion time < 30 seconds for 6 months of data (signal caching + batch processing)
...
1. **`src/walltrack/core/feedback/backtest_models.py`**
   - Complete Pydantic models for backtest functionality
   - `BacktestStatus`, `ExitStrategyType` enums
   - `ScoringWeights` with validation (must sum to 1.0)
   - `BacktestConfig`, `PositionSizingConfig`, `ExitStrategyConfig`
   - `HistoricalSignal`, `SimulatedTrade` with computed PnL fields
   - `PerformanceMetrics` with win rate, profit factor, expectancy
   - `MetricsComparison` for actual vs simulated comparison
   - `TradeComparison` for trade-by-trade analysis
   - `BacktestProgress`, `BacktestResult`
   - `ApplySettingsRequest`, `ApplySettingsResult`

2. **`src/walltrack/core/feedback/backtester.py`**
   - `BacktestService` with signal caching (30-min TTL)
   - `run_backtest()` - main entry point with progress callback
   - `_load_historical_signals()` - cached signal loading
   - `_load_price_histories()` - batch price data loading
   - `_rescore_signal()` - apply new weights to signal
   - `_simulate_trades()` - generate simulated trades
   - `_simulate_exit()` - exit strategy simulation (stop-loss, take-profit, trailing, time-based)
   - `_calculate_actual_metrics()` - metrics from actual trades
   - `_calculate_simulated_metrics()` - metrics from simulated trades
   - `_calculate_max_drawdown()` - drawdown calculation
   - `_calculate_max_consecutive_losses()` - consecutive loss tracking
   - `_build_trade_comparisons()` - compare actual vs simulated
   - `apply_settings()` - apply backtest settings to production
   - Singleton pattern via `get_backtest_service()`

3. **`src/walltrack/api/routes/backtest.py`**
   - `POST /backtest/run` - Run backtest with configuration
   - `GET /backtest/progress/{id}` - Get running backtest progress
   - `GET /backtest/result/{id}` - Get completed backtest result
   - `GET /backtest/history` - List recent backtest results
   - `POST /backtest/apply` - Apply settings from backtest
   - `GET /backtest/defaults` - Get default configuration
   - `POST /backtest/cache/clear` - Clear signal cache

4. **`src/walltrack/data/supabase/migrations/017_backtest.sql`**
   - `backtest_results` table with config, metrics, trades
   - `signal_cache` table with TTL-based expiration
   - `token_price_history` table for historical prices
   - `applied_settings_history` for audit trail
   - Helper functions: `cleanup_expired_signal_cache()`, `get_or_create_signal_cache()`, `update_backtest_progress()`
   - RLS policies and triggers

5. **`tests/core/feedback/test_backtester.py`**
   - 41 comprehensive tests covering all models and service methods
   - `ChainableMock` for Supabase async client testing
   - Tests for scoring weight validation, metrics calculation, exit simulation, backtest execution

### Key Features

- **Signal Caching**: 30-minute TTL cache for historical signals to ensure < 30s completion
- **Batch Processing**: Price histories loaded in batches for efficiency
- **Exit Simulation**: Supports stop-loss, take-profit, trailing stop, and time-based exits
- **Computed Fields**: Extensive use of Pydantic computed fields for derived metrics
- **Progress Tracking**: Real-time progress updates during backtest execution
- **Settings Application**: Apply validated settings with confirmation and rollback support

### Test Results

All 41 tests pass covering:
- Model validation and computed fields
- Exit strategy simulation
- Metrics calculation (win rate, profit factor, expectancy, drawdown)
- Trade comparison logic
- Service singleton pattern
- Full backtest execution flow
