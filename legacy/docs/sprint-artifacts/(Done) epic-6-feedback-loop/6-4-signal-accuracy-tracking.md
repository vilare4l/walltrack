...
- [x] Create `src/walltrack/core/feedback/accuracy_tracker.py`
- [x] Calculate signal-to-win rate
- [x] Compare scores of winning vs losing signals
- [x] Implement retrospective analysis for non-traded signals
- [x] Calculate accuracy trend over time
- [x] Break down accuracy by factor

## Definition of Done

- [x] Signal accuracy calculated correctly
- [x] Retrospective analysis identifies opportunities/bullets
- [x] Accuracy trend visualizable
- [x] Factor breakdown shows effectiveness
...
1. **`src/walltrack/core/feedback/accuracy_models.py`** - Pydantic models:
   - `AccuracyTrend` - Trend direction enum (IMPROVING, STABLE, DECLINING)
   - `RetrospectiveOutcome` - Classification for non-traded signals
   - `SignalAccuracyMetrics` - Core accuracy metrics with computed fields
   - `ThresholdAnalysis` - Threshold effectiveness analysis
   - `RetrospectiveSignal` / `RetrospectiveAnalysis` - Retrospective models
   - `AccuracySnapshot` / `AccuracyTrendAnalysis` - Trend analysis models
   - `FactorAccuracyBreakdown` - Factor-level accuracy breakdown

2. **`src/walltrack/core/feedback/accuracy_tracker.py`** - Service implementation:
   - `AccuracyTracker` class with caching support for testing
   - `calculate_accuracy_metrics()` - Win rate, trade rate, score stats
   - `analyze_thresholds()` - Multi-threshold analysis (0.40-0.85)
   - `run_retrospective_analysis()` - Missed opportunities / bullets dodged
   - `analyze_trend()` - Linear regression-based trend detection
   - `breakdown_by_factor()` - Per-factor accuracy breakdown
   - `get_accuracy_tracker()` - Singleton accessor

3. **`src/walltrack/api/routes/accuracy.py`** - FastAPI routes:
   - `GET /accuracy/metrics` - Get accuracy metrics
   - `GET /accuracy/thresholds` - Analyze threshold effectiveness
   - `GET /accuracy/trend` - Get trend analysis
   - `GET /accuracy/factors` - Get factor breakdown
   - `GET /accuracy/retrospective` - Run retrospective analysis

4. **`src/walltrack/data/supabase/migrations/014_accuracy_tracking.sql`** - Schema:
   - `accuracy_snapshots` - Point-in-time metrics for trend analysis
   - `threshold_analyses` - Threshold effectiveness data
   - `factor_accuracy` - Factor breakdown data
   - `retrospective_signals` / `retrospective_summaries` - Retrospective data
   - RLS policies and cleanup function

5. **`tests/core/feedback/test_accuracy_tracker.py`** - 23 comprehensive tests

### Key Design Decisions

- Uses `statistics.linear_regression` for trend slope calculation
- Configurable thresholds (0.40-0.85 with 0.05 step)
- Default take profit 20%, stop loss 10% for PnL estimation
- In-memory caching pattern for testing without database
- ChainableMock pattern for Supabase async client mocking
