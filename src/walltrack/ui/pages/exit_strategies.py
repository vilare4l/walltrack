...
from walltrack.services.exit.strategy_templates import get_template
...
class ExitStrategiesPage:
    ...
    def __init__(self) -> None:
        self.service: ExitStrategyService | None = None
        ...
        try:
            ...
            rules = [
                ExitStrategyRule(
                    rule_type=r["rule_type"],
                    trigger_pct=(
                        Decimal(str(r["trigger_pct"]))
                        if r.get("trigger_pct") is not None
                        else None
                    ),
...
def generate_preview_chart(rules_str: str, entry_price: float = 100) -> plt.Figure | None:
...
def create_exit_strategies_page() -> None:  # noqa: PLR0915
        ...
        with gr.Column(scale=2):
            ...
            with gr.Group():
                with gr.Row():
                    ...
                    strategy_version = gr.Number(
                        label="Version", precision=0, interactive=False
                    )
...
- `params`: Additional parameters for rule type
    ...
    async def on_strategy_select(
        evt: gr.SelectData, table_data: list[list]
    ) -> list:
        if evt.index[0] < len(table_data):
            ...
            if "error" in details:
                return [None, "", "", 0, "", DEFAULT_RULES_JSON, 24, 6, 2.0, details["error"]]
        ...
        return [None, "", "", 0, "", DEFAULT_RULES_JSON, 24, 6, 2.0, ""]
    ...
    async def on_save(
        ...
        if strategy_id:
            _, msg = await page.update_strategy(
                strategy_id,
                name,
                description,
                rules_str,
            ...
            _, msg = await page.create_strategy(
                name,
                description,
                rules_str,
                int(max_hold),
    ...
    async def on_activate(strategy_id: str | None) -> str:
        if not strategy_id:
            return "No strategy selected"
        _, msg = await page.activate_strategy(strategy_id)
        return msg
    ...
    async def on_clone(
        ...
        _, msg = await page.clone_strategy(strategy_id)
        strategies = await page.get_strategies_list()
    ...
    async def on_delete(
        ...
        if not strategy_id:
            return "No strategy selected", await page.get_strategies_list()
        _, msg = await page.delete_draft(strategy_id)
        strategies = await page.get_strategies_list()
        return msg, strategies

    async def on_new() -> list:
        return [
            None,
            "",
            "",
            ...
            6,
            2.0,
            "New strategy - fill in details",
        ]

    async def on_create_from_template(
        template: str | None,
    ) -> tuple[str, list[list[str | int]]]:
        if not template:
            return "Select a template", await page.get_strategies_list()
        _, msg = await page.create_from_template(template)
        strategies = await page.get_strategies_list()
        return msg, strategies
